...
static int __mm_camera_message_callback(int message, void *param, void *user_data){
	...
	switch(message){
		...
		case MM_MESSAGE_CAMCORDER_LOW_LIGHT_STATE :
		{
			handle->low_light_state = m->code;
			if( handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE] ){
				LOGI("_CAMERA_EVENT_TYPE_LIGHT_CHANGE=%d", handle->low_light_state);
				((camera_low_light_state_changed_cb)handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE])(handle->low_light_state, handle->user_data[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE]);
			}
			break;
		}
		case MM_MESSAGE_CAMCORDER_CAPTURE_SOUND:
		{
			LOGW("_CAMERA_EVENT_TYPE_SHUTTER_SOUND callback %p",
			     handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND]);
			if (handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND]) {
				((camera_shutter_sound_cb)handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND])(handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND]);
			}
			break;
		}
		case MM_MESSAGE_CAMCORDER_CAPTURE_SOUND_COMPLETED:
		{
			LOGW("_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED callback %p",
			     handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED]);
			if (handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED]) {
				((camera_shutter_sound_cb)handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED])(handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED]);
			}
			break;
		}
	}
...
static int __low_light_send_cb(void *data)
{
	camera_s *handle = (camera_s *)data;
	...
	if (handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE]) {
		LOGI("call _CAMERA_EVENT_TYPE_LIGHT_CHANGE = %d", handle->low_light_state);
		((camera_low_light_state_changed_cb)handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE])(handle->low_light_state, handle->user_data[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE]);
	}
...
int camera_set_x11_display_pixmap(camera_h camera, camera_x11_pixmap_updated_cb callback, void *user_data)
{
	...
	if (handle == NULL || callback == NULL || user_data == NULL) {
	...
	ret = mm_camcorder_set_attributes(handle->mm_handle, NULL,
	                                  ...
	                                  MMCAM_DISPLAY_HANDLE, callback, sizeof(callback),
	                                  MMCAM_DISPLAY_HANDLE_USER_DATA, user_data, sizeof(user_data),
	                                  NULL);
...
int camera_set_low_light_state_changed_cb(camera_h camera, camera_low_light_state_changed_cb callback, void* user_data){
	...
	camera_s * handle = (camera_s*)camera;
	handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE] = (void*)callback;
	handle->user_data[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE] = (void*)user_data;

	/* send current low light status */
	g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, __low_light_send_cb, handle, NULL);
...
int camera_unset_low_light_state_changed_cb(camera_h camera){
	...
	camera_s * handle = (camera_s*)camera;
	handle->user_cb[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE] = (void*)NULL;
	handle->user_data[_CAMERA_EVENT_TYPE_LOW_LIGHT_STATE_CHANGE] = (void*)NULL;
	return CAMERA_ERROR_NONE;
}
...
int camera_set_shutter_sound_cb(camera_h camera, camera_shutter_sound_cb callback, void* user_data)
{
	...
	handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND] = (void *)callback;
	handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND] = (void *)user_data;
...
int camera_unset_shutter_sound_cb(camera_h camera)
{
	...
	handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND] = (void *)NULL;
	handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND] = (void *)NULL;
...
int camera_set_shutter_sound_completed_cb(camera_h camera, camera_shutter_sound_completed_cb callback, void* user_data)
{
	...
	handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED] = (void *)callback;
	handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED] = (void *)user_data;
...
int camera_unset_shutter_sound_completed_cb(camera_h camera)
{
	...
	handle->user_cb[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED] = (void *)NULL;
	handle->user_data[_CAMERA_EVENT_TYPE_SHUTTER_SOUND_COMPLETED] = (void *)NULL;
...
int camera_get_recommended_preview_resolution(camera_h camera, int *width, int *height){
	...
	if (width && width_info.int_array.count > wide) {
	...
	if (height && height_info.int_array.count > wide) {
...
int camera_attr_set_auto_exposure_lock(camera_h camera, bool lock)
{
	...
	ret = mm_camcorder_set_attributes(handle->mm_handle, NULL, MMCAM_AUTO_EXPOSURE_LOCK, lock, NULL);
...
int camera_attr_get_auto_exposure_lock(camera_h camera, bool *lock)
{
	...
	if (handle == NULL || lock == NULL) {
		LOGE("INVALID_PARAMETER handle %p, lock %p", handle, lock);
		return CAMERA_ERROR_INVALID_PARAMETER;
	}

	ret = mm_camcorder_get_attributes(handle->mm_handle, NULL, MMCAM_AUTO_EXPOSURE_LOCK, &ae_lock, NULL);
	if (ret == MM_ERROR_NONE) {
		if (ae_lock) {
			*lock = true;
		} else {
...
int camera_attr_set_auto_white_balance_lock(camera_h camera, bool lock)
{
	...
	ret = mm_camcorder_set_attributes(handle->mm_handle, NULL, MMCAM_AUTO_WHITE_BALANCE_LOCK, lock, NULL);
...
int camera_attr_get_auto_white_balance_lock(camera_h camera, bool *lock)
{
	...
	ret = mm_camcorder_get_attributes(handle->mm_handle, NULL, MMCAM_AUTO_WHITE_BALANCE_LOCK, &awb_lock, NULL);
	if (ret == MM_ERROR_NONE) {
		if (awb_lock) {
			*lock = true;
		} else {
...
int camera_attr_get_shutter_sound(camera_h camera, camera_shutter_sound_e *sound)
{
	...
	if (camera == NULL || sound == NULL) {
		LOGE("[%s] INVALID_PARAMETER(0x%08x)", __func__, CAMERA_ERROR_INVALID_PARAMETER);
		return CAMERA_ERROR_INVALID_PARAMETER;
	}

	ret = mm_camcorder_get_attributes(handle->mm_handle, NULL,
	                                  MMCAM_CAPTURE_SOUND, &result,
	                                  NULL);
	if (ret == 0){
		*sound = result;
	}
